...
struct BrowseFolder: Identifiable {
    let id = UUID()
    let name: String
...
final class BrowseViewModel: ObservableObject {
    @Published private(set) var sections: [(header: String, items: [BrowseItem])] = []
    ...
    func loadFolders() {
        ...
        session.info(url, credential: space.credential) { [weak self] info, error in
            DispatchQueue.main.async {
                ...
                if let error = error {
                    self.sections = [(header: "", items: [.error(error)])]
                    return
                }
                ...
                self.sections = [(header: "", items: folders.map { .folder($0) })]
            }
        }
    }
    
    func addSelectedFolder() {
