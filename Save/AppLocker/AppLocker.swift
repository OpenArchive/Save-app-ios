...
public class AppLocker: UIViewController {
    ...
    static let valet = Valet.valet(with: Identifier(nonEmpty: "Save")!, accessibility: .whenUnlockedThisDeviceOnly)
    // MARK: - Pincode
    ...
    private var savedPin: String? {
        get {
            try? AppLocker.valet.string(forKey: ALConstants.kPincode)
        }
        set {
    ...
    private func validateModeAction() {
        log.debug("saved pin = \(savedPin.orNil)")
        if pin == savedPin {
...
public extension AppLocker {
    // Present AppLocker
    class func present(with mode: ALMode, and config: ALOptions? = nil, over viewController: UIViewController? = nil, animated: Bool = true) {
